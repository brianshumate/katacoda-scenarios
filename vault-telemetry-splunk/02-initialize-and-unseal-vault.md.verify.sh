...
if [[ $(docker ps -f name=vtl-vault --format "{{.Status}}" | grep -qw healthy) ]]
  then
    ...
    echo "Vault server container is not healthy. Please ensure it is healthy, and that you have initialized and unsealed it before continuing."  >> "$HOME"/02-init-and-unseal.log 2>&1
fi
...
if [[ $(vault status -format=json | jq -r '.initialized' | grep -q true) ]]
  then
    ...
    echo "Vault server is not initialized. Please initialize it, unseal it, and login before continuing."  >> "$HOME"/02-init-and-unseal.log 2>&1
fi
